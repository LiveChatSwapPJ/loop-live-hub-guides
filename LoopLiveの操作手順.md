...
| 5 | `ユーザー編集` | `権限` を選択する（一般 / クリエイター） | 権限の選択が反映される |
...
| 10 | 確認ダイアログ | `はい` を押す | `加工処理中…`（または一時的に `接続中...`）になる |
| 11 | `StreamingPage` | `加工停止` が表示されるまで待つ | 加工が開始される |
| 12 | `StreamingPage` | 必要に応じて `加工後の映像を見る` を押す | 別タブで加工映像が表示される |
| 13 | `StreamingPage` | `加工停止` を押す | ボタンが `加工開始` に戻る |

#### B-2-3. 補足（所要時間）

- **起動の確認〜準備完了（手順4〜5）**: `はい` 押下後、`準備完了` になるまで数分〜十数分かかることがある。
- **加工の確認〜加工処理中（手順10〜11）**: `はい` 押下後、`加工処理中…` になるまで数十秒〜数分かかることがある。
